                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip() == "/debug":
                            ...
                            debug_msg = f"[FF0000]✅MOMIN ONLINE BOT! UID: {uid}"
...
[FFFFFF]          ⚡MOMIN ONLINE BOT❤️  
[FFFFFF]                   thank for support 
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]MOMIN ONLINE BOT    
[FFD700]✨ Name on instagram : [FFFFFF]MOMIN ONLINE BOT❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </>MOMIN ONLINE BOT  ⚡"""
                            else:
                                message = f"""[C][B][FF0000]╔══════════╗
[FFFFFF]✨ folow on Instagram   
[FFFFFF]          ⚡MOMIN ONLINE BOT❤️  
...
[FFD700]✨ Developer —͟͞͞ </>MOMIN ONLINE BOT  ⚡"""
                        ...
                        if inPuTMsG.startswith('/eb '):
                            update_command_stats("eb")
                            try:
                                ...
                                if len(parts) >= 4:  # Minimum: /ee team_code uid1 uid2 emote_id
                                    ...
                                    if len(uids) >= 1 and emote_id:
                                        ...
                                        error_msg = f"[FF0000]Usage: /eb [TEAM_CODE] [UID1] [UID2] [UID3] [EMOTE]\nExample: /eb FAST123 1234 5678 9012 1\n[B22222]Will auto-join team and leave after emotes"
                                ...
                                error_msg = f"[FF0000]/eb command error"
                                P = await SEndMsG(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                            continue
...
async def MaiiiinE():
    ...
    Uid , Pw = '4317651789','D212AC2BB039A6F1268DEC0DED1B1CF83C470A4FCA5AE6F46F28648BD47FAE64'
    ...
    print('')
    print(f" -MOMIN ONLINE BOT STarTinG And OnLine on TarGet : {TarGeT} | BOT NAME : {acc_name}\n")
    print(f" - BoT sTaTus > GooD | OnLinE ! (:")    
    print(f" -MOMIN ONLINE BOT | Bot Uptime: {time.strftime('%H:%M:%S', time.gmtime(time.time() - bot_start_time))}")    
